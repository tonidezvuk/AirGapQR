...
class AirGapApp(QMainWindow):
    def __init__(self):
        ...
        self.setWindowIcon(QIcon(resource_path("AirGapQR_icon.png")))
        screen = QApplication.primaryScreen()

        if screen is not None:
            available = screen.availableGeometry()

            window_width = min(1500, max(900, available.width() - 80))
            window_height = min(920, max(600, available.height() - 80))
    ...
    def _detect_cameras(self):
        ...
        for device in sorted(Path("/dev").glob("video*")):
            try:
                index = int(device.name.replace("video", ""))
            except ValueError:
                continue
